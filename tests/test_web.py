...
def test_pwa_assets_from_the_repo_root(tmp_path, monkeypatch):
    c = client(tmp_path, monkeypatch)
    r = c.get("/")
    assert r.status_code == 200 and "CHORDWARE" in r.text and "__BUILD__" not in r.text
    assert r.headers["cache-control"] == "no-store"
    assert "__BUILD__" not in c.get("/sw.js").text
    m = c.get("/manifest.webmanifest").json()
    assert m["id"] == "/chordware" and m["share_target"]["action"] == "share"
    assert c.get("/icon-192.png").headers["content-type"] == "image/png"
    assert c.get("/nope.txt").status_code == 404 and c.get("/tabsmith/").status_code == 404
...
def test_share_target_and_bad_capo(tmp_path, monkeypatch):
    ...
    assert r.status_code == 303 and r.headers["location"].startswith("/#audio/")
