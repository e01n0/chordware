...
- **Learn** — technique lessons as animated tab (the LEARN chip). **Rolls**: five banjo right-hand patterns (forward, backward, forward-reverse, alternating thumb, pinch) for any 4-string tuning — a roll is string order + timing, so pick any chord from the library and the tab's frets follow what you're holding, with T/I/M finger badges. **Slide licks**: authored phrases per tuning (open-G blues set for DGBD: the ♭3 slide, double-stop slide to the IV, ♭5 wobble, turnaround walk-down), slides drawn as connected fret pairs and sounded as chromatic re-attack runs. Loop playback sweeps a playhead across the tab at 40–240 BPM, lighting each note as it sounds; SND toggles between hearing the pattern and a bare metronome tick to play over
